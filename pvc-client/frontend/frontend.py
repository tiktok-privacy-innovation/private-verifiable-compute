...
# Copy from pve-tee-lmm/Rocket.toml
openai_api_key = os.getenv("LLM_API_KEY", "")
openai_api_base = os.getenv("LLM_ENDPOINT", "http://127.0.0.1:8083/v1")
openai_api_model = os.getenv("LLM_MODEL", "Qwen/Qwen3-VL-4B-Thinking")
...
def get_llm_response(messages, model, reason, content_ph=None, reasoning_ph=None):
    ...
    Args:
        messages (list): List of conversation message dicts with '' and 'content'
...
def server_supports_reasoning():
    if openai_api_model == 'Qwen/Qwen3-VL-4B-Thinking':
        return False
