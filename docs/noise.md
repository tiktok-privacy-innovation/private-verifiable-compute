...
The first step is a standard attestation report verification, and the extra step is to bind the responder Noise identity to that attestation.
PVC now does that with an explicit handshake contract:

- `/v1/attestation` accepts JSON `{ "nonce": "<base64 64-byte value>" }` and returns `attestation.cpu` plus optional `attestation.devices`.
- `/v1/handshake` returns the same normalized attestation envelope plus `binding.handshake_verifying_key` and `session.id`.
- `binding.handshake_verifying_key` is the verifier-visible public key for the signature key that protects the Noise transcript.

The handshake verifying key is generated by the PVC compute node such that the corresponding signing key is only accessible within the compute node.
The client verifies the attestation envelope first, reads `binding.handshake_verifying_key` from the verified handshake response, and only then starts Noise. Finally, the PVC node includes a signature in the last round of communication, and the signed message is the entire transcript of the messages sent by both parties.
...
1. PVC Compute Node
    - Generate a signature key pair `(pk, sk)` for the Noise transcript.
    - Produce an attestation envelope whose CPU evidence covers the handshake verifying key and whose device evidences are returned alongside it.
    - Return that envelope from `/v1/handshake` together with `binding.handshake_verifying_key` and `session.id`.

2. Client
    - Verify `attestation.cpu` and any `attestation.devices`.
    - Treat `binding.handshake_verifying_key` as the canonical binding material for the responder identity.
    - Start Noise only after the binding material has been verified and accepted.
